...
public:
    ...
    lighthouse_console &operator=(const lighthouse_console &) = delete;

    // Device queries
    std::vector<std::string> list_connected_dongles();
    std::vector<std::string> list_paired_dongles();
    bool is_dongle_paired(const std::string &serial);

    // Single device operations
    void pair_dongle(const std::string &serial);
    void unpair_dongle(const std::string &serial);
    void power_off_dongle(const std::string &serial);

    // Bulk operations
    void pair_all_dongles();
    void unpair_all_dongles();
    void force_pair_all_dongles();
...
private:
    ...
    void select_dongle(const std::string &serial);
    bool wait_for_prompt(std::string &output);
    std::string read_until_prompt();
    bool is_process_alive() const;

    std::vector<dongle_info> parse_dongle_list(const std::string &response);
};